                        ...
                        for($j = 0; $j < count($_obj[$i]['param']); $j++){
                            ...
                            switch($_type){
                                ...
                                case 'integer':
                                    echo($_param[$j]);
                                    # 创建get参数对象信息
                                    $_GET[$_obj[$i]['param'][$j]['name']] = ((!empty($_param[$j]) and intval($_param[$j]) != 0 )or !is_null($_param[$j])) ? intval($_param[$j]) : (is_null($_default)) ? null : intval($_default);
                                    break;
                                case 'float':
                                case 'double':
                                    # 创建get参数对象信息
                                    $_GET[$_obj[$i]['param'][$j]['name']] = (!empty($_param[$j]) or !is_null($_param[$j])) ? intval($_param[$j]) : (is_null($_default)) ? null : doubleval($_default);
                                    break;
                                case 'boolean':
                                    # 创建get参数对象信息
                                    $_GET[$_obj][$i]['param'][$j]['name'] = ((!empty($_param[$j]) and intval($_param[$j]) != 0 and boolval($_param[$j]) != false )or !is_null($_param[$j])) ? boolval($_param[$j]) : (is_null($_default)) ? null : boolval($_default);
                                    break;
                                case 'string':
                                default:
                                    # 创建get参数对象信息
                                    $_GET[$_obj[$i]['param'][$j]['name']] = (!empty($_param[$j]) or !is_null($_param[$j])) ? strval($_param[$j]) : (is_null($_default)) ? null : strval($_default);
                                    break;
                            }
                        }
                    }
